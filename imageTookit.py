# -*- coding: utf-8 -*-
"""
Created on Fri Oct 13 07:28:26 2023
...
def flipVertically(image):
    # Create an image that is the same size as the original
    width = image.width()
    height = image.height()
    newImage = GraphicsImage(width, height)
    ...
    for row in range(height):
        for col in range(width):
            ...
            newImage.setPixle(newRow, newCol, pixel)
        newRow = newRow - 1
    return(newImage)
...
def adjustBrightnes(image, amount):
    width = image.width()
    height = image.height()
    
    # Create a new image that is the sme size as the original size
    newImage = GraphicsImage(width, height)
    for row in range(height):
        for col in range(width):
